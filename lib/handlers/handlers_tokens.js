...
handlers._tokens.post = (data, callback) => {
    ...
    _data.read('users', email, (err, userData) => {
        ...
        const tokenObject = {
            ...
            expires: Date.now() + config.tokenExpirationPeriod,
        }
...
handlers._tokens.put = (data, callback) => {
    ...
    _data.read('tokens', tokenID, (err, tokenData) => {
        ...
        tokenData.expires = Date.now() + config.tokenExpirationPeriod;
...
handlers._tokens.verifyToken = (tokenID, email, callback) => {
    _data.read('tokens', tokenID, (err, tokenData) => {
        if (err || !tokenData) {
            return callback(false, 'Could not read token file');
        }
        if (tokenData.email !== email) {
            return callback(false, 'Token email does not match');
        }
        if (tokenData.expires < Date.now()) {
            return callback(false, 'Token is expired');
        }
